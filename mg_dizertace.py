...
while not(found_best):
    ...
    for index, value in np.ndenumerate(mackey_glass_data):
        if index[0] > mg_t:
            ...
            filter_data[index[0] - mg_t, 4] = mackey_glass_data[index[0] - 1] * mackey_glass_data[index[0] - 2]
            filter_data[index[0] - mg_t, 5] = mackey_glass_data[index[0] - 1] * mackey_glass_data[index[0] - 3]
            filter_data[index[0] - mg_t, 6] = mackey_glass_data[index[0] - 1] * mackey_glass_data[index[0] - 4]
            filter_data[index[0] - mg_t, 7] = mackey_glass_data[index[0] - 2] * mackey_glass_data[index[0] - 3]
            filter_data[index[0] - mg_t, 8] = mackey_glass_data[index[0] - 2] * mackey_glass_data[index[0] - 4]
            filter_data[index[0] - mg_t, 9] = mackey_glass_data[index[0] - 3] * mackey_glass_data[index[0] - 4]
            filter_data[index[0] - mg_t, 10] = mackey_glass_data[index[0] - 1]
            filter_data[index[0] - mg_t, 11] = mackey_glass_data[index[0] - 2]
            filter_data[index[0] - mg_t, 12] = mackey_glass_data[index[0] - 3]
            filter_data[index[0] - mg_t, 13] = mackey_glass_data[index[0] - 4]
            filter_data[index[0] - mg_t, 14] = 1
    ...
    if min_index > 523 - mg_t and min_index < 526 - mg_t:
        if np.argmax(elbnd) > 600:
            found_best = True
            print('seed:', my_seed)
    found_best=True


np.savetxt("foo_diz.csv", hpp, delimiter=",")
...
plt.annotate('pertubovaný vzorek', xy=(523, 1.2), xytext=(523, 1.4), arrowprops=dict(facecolor='black', shrink=1))
ax2 = plt.subplot(223)
# print('mg shape', mackey_glass_data[316:].shape)
# print('pertubed shape', pertubed_mackey_glass[316:].shape)
ax2.plot(x, mackey_glass_data[316:], color='b', linewidth=1, label='bez pretubace',)
ax2.plot(x, pertubed_mackey_glass[316:], color='k', linewidth=1,label='partubovaná data',  linestyle='dashed')
...
ax3.plot(x, mackey_glass_data[316:], color='k', label='bez pretubace')
ax3.plot(x, pertubed_mackey_glass[316:], color='k', linewidth=1, linestyle='--', marker='*', markersize=9, label='pertubovaná řada')
...
plt.annotate('pertubovaný vzorek', xy=(523, -1.0), xytext=(523, -1.1), arrowprops=dict(facecolor='black', shrink=1))


plt.savefig('mackeydetails_diz.eps', format='eps', dpi=300)
# ND
...
plt.tight_layout()
plt.savefig('mackey_results_diz.eps', format='eps', dpi=300)
...
plt.ylabel("$LE$ $[-]$")
plt.tight_layout()
plt.savefig('mackey_results_nd_diz.eps', format='eps', dpi=300)
"""
plt.figure(10)
plt.plot((hpp))
plt.title('hpp2')
