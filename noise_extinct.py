...
import numpy as np
from pot import pot_min
from datetime import datetime
from scipy.stats import genpareto
import matplotlib.pyplot as plt

found_best = False
seed_counter = 515 #516
while not(found_best):
    ...
    for idx in range(experiment_len):
        ...
        if idx < parameter_change_idx:
            desired_output[idx] = x[idx, 0] + x[idx, 1] + x[idx, 0] * x[idx, 1] + np.random.normal(0, 0.1, 1)


        else: # 0.4 1.6
            desired_output[idx] = x[idx, 0] + x[idx, 1] + x[idx, 0] * x[idx, 1]
    ...
    for i in range(gev_window, dw.shape[0]):
        ...
        for j in range(filter_len):
            poted_values = pot_min(dw[i-gev_window:i, j], 1)
    ...
    if min_index > 499 and min_index < 503:
        found_best = True
...
plt.savefig('noise_ext.eps', format='eps', dpi=300)
