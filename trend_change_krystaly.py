...
found_best = False
seed_counter = 95 #96, 156, 158 167 192 ok 195 TOP
while not(found_best):
    ...
    experiment_len = 13000
    inputs_number = 2
    filter_len = 3
    parameter_change_idx = 1000
    gev_window = 5000 #5000
    #
    ...
    for i in range(gev_window, dw.shape[0]):
        ...
        for j in range(filter_len):
            poted_values = pot(dw[i-gev_window:i, j], 3) #1
            if dw[i, j] > poted_values[-1]:
                fit = genpareto.fit(pot(dw[i-gev_window:i, j], 1), 1, loc=0, scale=1)
                if dw[i, j] >= fit[1]:
                    hpp[i-gev_window, j] = 1 - genpareto.cdf(dw[i, j], fit[0], fit[1], fit[2]) + 5e-45
                #print(fit)
    ...
    # if min_index > 499 and min_index < 503 and np.min(np.log10(totalhpp1[0:499])) > -20:
    #     found_best = True
...
#elbnd =[]# pa.detection.ELBND(w, w, function="sum")
#elbnd[0:2] = 0
...
plt.savefig('trendchange_krystaly.eps', format='eps', dpi=300)
