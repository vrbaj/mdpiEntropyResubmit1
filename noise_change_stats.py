...
for noise_sigma in noise_sigmas:
    ...
    for seed_counter in range(0, experiments_number):
        ...
        for idx in range(experiment_len):
            if idx == 0: #or idx == parameter_change_idx:
                ...
                # noise_sigma = 0# np.random.uniform(low=0.0, high=1, size=1)
                pass
            if idx == parameter_change_idx:
                noise_add = np.random.uniform(low=0.01, high=1, size=1)
            filter_data[idx, 0] = x[idx, 0]
            filter_data[idx, 1] = x[idx, 1]
            filter_data[idx, 2] = x[idx, 0] * x[idx, 1]
            if idx < parameter_change_idx:
                desired_output[idx] = x[idx, 0] + x[idx, 1] + x[idx, 0] * x[idx, 1] +  np.random.normal(0, noise_sigma, 1)
                noiseless_signal[idx] = x[idx, 0] + x[idx, 1] + x[idx, 0] * x[idx, 1]
            else: # 0.4 1.6
                desired_output[idx] = x[idx, 0] + x[idx, 1] + x[idx, 0] * x[idx, 1] +  np.random.normal(0, noise_sigma + noise_add, 1)
                noiseless_signal[idx] = x[idx, 0] + x[idx, 1] + x[idx, 0] * x[idx, 1]
    ...
    with open("noise_change.csv", mode="a", newline='') as results_file:
